...
const userSettingsSelect = `SELECT user_id, weight_unit, calorie_target, protein_target, carb_target, fat_target, rest_timer_normal, rest_timer_warmup, rest_timer_drop FROM user_settings`
...
func (s *UserStore) GetSettings(uid int64) (models.UserSettings, error) {
	...
	err := s.db.QueryRow(userSettingsSelect+` WHERE user_id = ?`, uid).
		Scan(&st.UserID, &st.WeightUnit, &st.CalorieTarget, &st.ProteinTarget, &st.CarbTarget, &st.FatTarget, &st.RestTimerNormal, &st.RestTimerWarmup, &st.RestTimerDrop)
	return st, err
}
...
func (s *UserStore) UpsertSettings(uid int64, req models.UpdateSettingsRequest) (models.UserSettings, error) {
	...
	var st models.UserSettings
	err := s.db.QueryRow(
		`INSERT INTO user_settings (user_id, weight_unit, calorie_target, protein_target, carb_target, fat_target, rest_timer_normal, rest_timer_warmup, rest_timer_drop)
		 VALUES (?, COALESCE(?, ?), COALESCE(?, ?), COALESCE(?, ?), COALESCE(?, ?), COALESCE(?, ?), COALESCE(?, ?), COALESCE(?, ?), COALESCE(?, ?))
		 ON CONFLICT(user_id) DO UPDATE SET
		   ...
		   rest_timer_normal = COALESCE(?, user_settings.rest_timer_normal),
		   rest_timer_warmup = COALESCE(?, user_settings.rest_timer_warmup),
		   rest_timer_drop   = COALESCE(?, user_settings.rest_timer_drop)
		 RETURNING user_id, weight_unit, calorie_target, protein_target, carb_target, fat_target, rest_timer_normal, rest_timer_warmup, rest_timer_drop`,
		uid,
		...
		req.RestTimerNormal, d.RestTimerNormal,
		req.RestTimerWarmup, d.RestTimerWarmup,
		req.RestTimerDrop, d.RestTimerDrop,
		req.WeightUnit, req.CalorieTarget, req.ProteinTarget, req.CarbTarget, req.FatTarget, req.RestTimerNormal, req.RestTimerWarmup, req.RestTimerDrop,
	).Scan(&st.UserID, &st.WeightUnit, &st.CalorieTarget, &st.ProteinTarget, &st.CarbTarget, &st.FatTarget, &st.RestTimerNormal, &st.RestTimerWarmup, &st.RestTimerDrop)
	if err != nil {
