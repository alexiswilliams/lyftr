...
func insertWorkoutExercises(tx *sql.Tx, wid int64, exercises []models.CreateWorkoutExerciseReq) error {
	for i, ex := range exercises {
		...
		for j, st := range ex.Sets {
			if _, err := tx.Exec(
				`INSERT INTO sets (workout_exercise_id, set_number, reps, weight, duration, distance, rpe, set_type, is_warmup, rest_seconds, tempo, isohold_seconds, timestamp_completed)
				 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
				weid, j+1, st.Reps, st.Weight, st.Duration, st.Distance, st.RPE, st.SetType, st.IsWarmup, st.RestSeconds, st.Tempo, st.IsoholdSeconds, st.TimestampCompleted,
			); err != nil {
				return err
			}
		}
...
func (s *WorkoutStore) loadSets(workoutExerciseID int64) ([]models.Set, error) {
	rows, err := s.db.Query(
		`SELECT id, workout_exercise_id, set_number, reps, weight, duration, distance, rpe, is_warmup, set_type, COALESCE(rest_seconds, 0), COALESCE(tempo, ''), COALESCE(isohold_seconds, 0), timestamp_completed
	...
	for rows.Next() {
		var st models.Set
		if err := rows.Scan(&st.ID, &st.WorkoutExerciseID, &st.SetNumber, &st.Reps, &st.Weight, &st.Duration, &st.Distance, &st.RPE, &st.IsWarmup, &st.SetType, &st.RestSeconds, &st.Tempo, &st.IsoholdSeconds, &st.TimestampCompleted); err != nil {
			return nil, err
		}
		sets = append(sets, st)
	}
...
func prForExerciseTx(q interface {
	...
	err := q.QueryRow(`
		...
		WHERE w.user_id = ? AND we.exercise_id = ? AND s.set_type = 'normal' AND s.weight > 0
...
func (s *WorkoutStore) HistoryForExercise(uid, exerciseID int64, limit int) ([]ExerciseHistoryPoint, error) {
	rows, err := s.db.Query(`
		...
		JOIN workout_exercises we ON we.id = s.workout_exercise_id
		JOIN workouts w ON w.id = we.workout_id
		WHERE w.user_id = ? AND we.exercise_id = ? AND s.set_type = 'normal'
		GROUP BY w.id
